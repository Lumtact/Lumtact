...
| 层级 | 技术栈 | 职责 |
| :--- | :--- | :--- |
| **数据引擎** | Rust (`dag-generator`) | 解析核心逻辑，生成结构化的拓扑数据（JSON），输出到 `docs/` |
| **表现层** | Next.js 16 (`web-viewer`) | 数据驱动的文档阅读器，负责渲染与交互 |
...
数据是 Lumtract 的起点。运行 Rust 引擎生成结构化数据：
...
执行后，终端会显示 `✅ Manifest generated: ../docs/dag-manifest.json`。

### 2. 分发数据（供前端访问）

Next.js 默认从 `public/` 目录提供静态文件，因此需要将生成的数据复制到前端目录：

```bash
# 回到项目根目录
cd ..
# 复制 manifest 到前端 public 目录
cp docs/dag-manifest.json web-viewer/public/dag-data.json
```
...
访问 [http://localhost:3000](http://localhost:3000) 查看生成的图谱结构。
